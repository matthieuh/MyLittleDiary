...
export const Post = ({
  ...
  return (
    <Card
      ...
      onLayout={(event) => {
        ...
        const count = Math.floor(
          (availableSpace - PICTURE_WIDTH) /
            (PICTURE_WIDTH - PICTURE_OVERLAPPING),
        )
    ...
    >
      {createdAt && (
        <XStack ai="baseline">
          <Text fontWeight="700" mb="$2" fontFamily="$mono">
      ...
      <YStack gap="$2">
        {!!medias.length && (
          <XStack>
            {medias
              .slice(0, nbPicturesDisplayable)
              .map(({ type, uri, width, height }, index) => {
                ...
                if (type === 'video')
                  return (
                    <View key={uri} {...mediaContainerProps}>
                      <VideoPlayer {...mediaProps} controllable={false} />
                    </View>
                  )
              })}
        ...
        {audio && (
          <XStack>
            <AudioPlayer {...audio} fontSize="$1" iconSize="$1" />
          </XStack>
        )}
