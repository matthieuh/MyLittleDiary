...
import Animated, { ReduceMotion, SlideInRight, useAnimatedStyle, useSharedValue, withTiming, ZoomIn } from "react-native-reanimated"
...
export const AudioRecorder = ({ height = 50, autoRecord = false, onStop }: AudioRecorderProps) => {
    ...
    const { recording } = await Audio.Recording.createAsync(Audio.RecordingOptionsPresets.HIGH_QUALITY,
      (status) => {
        if (status.isRecording && status.metering) {
          ...
          let temp1 = [...tempRef.current.temp, (status.metering + height)]
  ...
  return (
    <XStack gap="$2">
      ...
      <XStack px="$4" py="$2" bg="$blue9" gap="$2" br="$10" ai="center" f={1}>
        ...
        <Text color="$white1" fontWeight="700">{formatDuration(duration)}</Text>
      </XStack>
      <Button
        icon={<Save size="$1.5" />}
        px="$3"
